...
ArkInventory = LibStub("AceAddon-3.0"):NewAddon("ArkInventory", "AceConsole-3.0", "AceHook-3.0", "AceEvent-3.0",
	"AceBucket-3.0")
...
ArkInventory.IsCataclysm = false
local tocVersion = GetAddOnMetadata("ArkInventory", "Version")
local function getVersion(uiVersion)
    local major, minor, patch = uiVersion:match("(%d+)%.(%d+)%.(%d+)")
    if major and minor and patch then
        return tonumber(string.format("%s.%02d%02d", major, tonumber(minor), tonumber(patch)))
    end
    return nil
end
ArkInventory.Const = {
	...
	Program = {
		Name = "ArkInventory",
		Version = getVersion(tocVersion),
		UIVersion = tocVersion,
	...
	Bag = {
		Status = {
		     -- these need to be negative values,  do not use -1 (false)
			Unknown = -2,
			Active = -3,
			Empty = -4,
			Purchase = -5,
	...
	Slot = {
		Type = {
		   -- slot type numbers, do not change this order, just add new ones to the end of the list
			Unknown = 0,
			Bag = 1,
			Key = 3,
			Soulshard = 5,
	...
	InventorySlotName = { "HeadSlot", "NeckSlot", "ShoulderSlot", "BackSlot", "ChestSlot", "ShirtSlot", "TabardSlot",
	...
	Category = {
		...
		Code = {
			System = {
			  -- do NOT change the indicies - if you have to then see the ConvertOldOptions( ) function to remap it
				[401] = {
					["id"] = "SYSTEM_DEFAULT",
	...
	Skills =
	"ALCHEMY,BLACKSMITHING,ENCHANTING,ENGINEERING,JEWELCRAFTING,INSCRIPTION,LEATHERWORKING,TAILORING,HERBALISM,MINING,SKINNING,ARCHAEOLOGY,COOKING,FIRST_AID,FISHING",       -- primary craft, primary collect, secondary
...
ArkInventory.Global = {
	...
	Tooltip = {
		...
		Mount = nil,
		Rule = nil,
		WOW = { GameTooltip, ShoppingTooltip1, ShoppingTooltip2, ShoppingTooltip3, ItemRefTooltip,
...
function ArkInventory.OnInitialize()
	...
	if ArkInventory.Const.Program.Beta then
		ArkInventory.Global.Version = string.format("%s %s(%s)%s", ArkInventory.Global.Version, RED_FONT_COLOR_CODE,
			ArkInventory.Const.Program.Beta or "unknown beta version", FONT_COLOR_CODE_CLOSE)
	end
	...
	ArkInventory.Lib.Config:RegisterOptionsTable(ArkInventory.Const.Frame.Config.Blizzard, ArkInventory.Config.Blizzard,
		{ "arkinventory", "arkinv", "ai" })
	...
	-- wearing
	table.insert(ArkInventory.Global.Location[ArkInventory.Const.Location.Wearing].Bags,
		ArkInventory.Const.Offset.Wearing + 1)
	...
	-- mount
	table.insert(ArkInventory.Global.Location[ArkInventory.Const.Location.Mount].Bags,
		ArkInventory.Const.Offset.Mount + 1)
	--ArkInventory.Global.Location[ArkInventory.Const.Location.Mount].bagCount = 1

	-- token
	table.insert(ArkInventory.Global.Location[ArkInventory.Const.Location.Token].Bags,
		ArkInventory.Const.Offset.Token + 1)
...
function ArkInventory.OnEnable()
	...
	ArkInventory:RegisterBucketMessage("LISTEN_VAULT_UPDATE_BUCKET",
		ArkInventory.db.global.option.bucket[ArkInventory.Const.Location.Vault] or 1.5)
...
function ArkInventory.ClassColourRGB(class)
	...
	local c = { r = ct.r <= 1 and ct.r >= 0 and ct.r or 0, g = ct.g <= 1 and ct.g >= 0 and ct.g or 0,
		b = ct.b <= 1 and ct.b >= 0 and ct.b or 0 }
...
function ArkInventory.LocationPlayerInfoGet(loc_id)
	...
	if loc_id == ArkInventory.Const.Location.Vault then
		...
		if guild_id then
			...
			if cp == nil then
				ArkInventory.Output("player id (", player_id, ") has an invalid guild id (", guild_id, ") at location (",
					loc_id, ")")
				assert(false, "code error")
			end
		end
...
function ArkInventory.SortKeyCustomAdd(name)
	...
	if n == -2 then
		ArkInventory.OutputError(
		"your data was recently upgraded, a ui reload is required before you can add a sort method")
		return
	end
	...
	for tn, d in pairs(categories) do
		for k, v in pairs(d) do
			...
			if name then
				...
				assert(not ArkInventory.Global.Category[cat_id],
					string.format("duplicate category: %s [%s] ", tn, cat_id))
...
function ArkInventory.CategoryCustomAdd(name)
	...
	if n == -2 then
		ArkInventory.OutputError(
		"your data was recently upgraded, a ui reload is required before you can add a custom category")
		return
	end
...
function ArkInventory.ItemCategoryGetDefaultActual(i)
	...
	local _, _, req = ArkInventory.TooltipFind(ArkInventory.Global.Tooltip.Scan,
		ArkInventory.Localise["WOW_TOOLTIP_CLASS"], false, true, true)
	if req then
	...
	if itemType == ArkInventory.Localise["WOW_ITEM_TYPE_TRADE_GOODS"] then
		...
		local t =
		"DEVICES,EXPLOSIVES,PARTS,HERB,CLOTH,ELEMENTAL,LEATHER,MEAT,METAL_AND_STONE,MATERIALS,ENCHANTMENT_ARMOR,ENCHANTMENT_WEAPON"
	...
	local _, _, req = ArkInventory.TooltipFind(ArkInventory.Global.Tooltip.Scan,
		ArkInventory.Localise["WOW_TOOLTIP_SKILL"], false, true, true)
	if req then
...
function ArkInventory.Frame_Main_Paint(frame)
	...
	for _, z in pairs({ frame:GetChildren() }) do
		...
		if framename then -- only process objects with a name (other addons can add frames without names, we don't want to deal with them)
			...
			if obj then
				local style = ArkInventory.LocationOptionGet(loc_id, "window", "background", "style") or
				ArkInventory.Const.Texture.BackgroundDefault
				if style == ArkInventory.Const.Texture.BackgroundDefault then
					...
					local file = ArkInventory.Lib.SharedMedia:Fetch(ArkInventory.Lib.SharedMedia.MediaType.BACKGROUND,
						style)
					obj:SetTexture(file)
				end
			end
			...
			if obj then
				if ArkInventory.LocationOptionGet(loc_id, "window", "border", "style") ~= ArkInventory.Const.Texture.BorderNone then
					local style = ArkInventory.LocationOptionGet(loc_id, "window", "border", "style") or
					ArkInventory.Const.Texture.BorderDefault
					local file = ArkInventory.Lib.SharedMedia:Fetch(ArkInventory.Lib.SharedMedia.MediaType.BORDER, style)
					local size = ArkInventory.LocationOptionGet(loc_id, "window", "border", "size") or
					ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].size
					local offset = ArkInventory.LocationOptionGet(loc_id, "window", "border", "offset") or
					ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].offset
					...
					ArkInventory.Frame_Border_Paint(obj, false, file, size, offset, scale, colour.r, colour.g, colour.b,
						1)
...
function ArkInventory.Frame_Main_OnLoad(frame)
	...
	assert(loc_id ~= nil,
		string.format("xml element '%s' is not an %s frame", framename, ArkInventory.Const.Program.Name))
	...
	for k, v in pairs(ArkInventory.Const.Actions) do
		local obj = _G
		[string.format("%s%s%s%s", frame:GetName(), ArkInventory.Const.Frame.Title.Name, "ActionButton", k)]
...
function ArkInventory.Frame_Container_CalculateBars(frame, Layout)
			...
			if not ignore then
				...
				if not hidden then
					...
					if not Layout.bar[bar_id] then
						Layout.bar[bar_id] = { ["id"] = bar_id, ["item"] = {}, ["count"] = 0, ["width"] = 0,
							["height"] = 0, ["ghost"] = false, ["frame"] = 0 }
					end
	...
	if ArkInventory.Global.Mode.Edit or ArkInventory.LocationOptionGet(loc_id, "bar", "showempty") then
		...
		for bar_id = 1, Layout.bar_count do
			if not Layout.bar[bar_id] then
				-- create a ghost bar
				Layout.bar[bar_id] = { ["id"] = bar_id, ["item"] = {}, ["count"] = 1, ["width"] = 0, ["height"] = 0,
					["ghost"] = true, ["frame"] = 0 }
			else
...
function ArkInventory.Frame_Container_Draw(frame)
	...
	local pad_label = (ArkInventory.LocationOptionGet(loc_id, "bar", "name", "show") and ArkInventory.LocationOptionGet(loc_id, "bar", "name", "height")) or
	0
	...
	for rownum, row in ipairs(ArkInventory.Global.Location[loc_id].Layout.container.row) do
		...
		for bar_index, bar_id in ipairs(row.bar) do
			...
			if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
				local obj_width = bar.width * ArkInventory.Global.BAG_SLOT_SIZE + (bar.width - 1) * pad_slot +
				pad_bar_int * 2
				...
				row["height"] = bar.height * ArkInventory.Global.BAG_SLOT_SIZE + (bar.height - 1) * pad_slot +
				pad_bar_int * 2 + pad_label
				...
				if bar.frame == 1 then
					if anchor == ArkInventory.Const.Anchor.BottomLeft then
						obj:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", pad_window + pad_bar_ext,
							pad_window + pad_bar_ext)
					elseif anchor == ArkInventory.Const.Anchor.TopLeft then
					...
					elseif anchor == ArkInventory.Const.Anchor.TopRight then
						obj:SetPoint("TOPRIGHT", frame, "TOPRIGHT", 0 - pad_window - pad_bar_ext,
							0 - pad_window - pad_bar_ext)
					else -- if anchor == ArkInventory.Const.Anchor.BottomRight then
						obj:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0 - pad_window - pad_bar_ext,
							pad_window + pad_bar_ext)
					end
				else
					if bar_index == 1 then
						...
						local parent = string.format("%s%s%s", name, "Bar",
							ArkInventory.Global.Location[loc_id].Layout.bar[prev].frame)
...
function ArkInventory.Frame_Bar_Paint_All()
	...
	for loc_id, loc_data in ipairs(ArkInventory.Global.Location) do
		c = _G
		[string.format("%s%s%s", ArkInventory.Const.Frame.Main.Name, loc_id, ArkInventory.Const.Frame.Container.Name)]
...
function ArkInventory.Frame_Bar_Paint(frame)
	...
	if obj then
		if ArkInventory.LocationOptionGet(loc_id, "bar", "border", "style") ~= ArkInventory.Const.Texture.BorderNone then
			local style = ArkInventory.LocationOptionGet(loc_id, "bar", "border", "style") or
			ArkInventory.Const.Texture.BorderDefault
			local file = ArkInventory.Lib.SharedMedia:Fetch(ArkInventory.Lib.SharedMedia.MediaType.BORDER, style)
			local size = ArkInventory.LocationOptionGet(loc_id, "bar", "border", "size") or
			ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].size
			local offset = ArkInventory.LocationOptionGet(loc_id, "bar", "border", "offset") or
			ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].offset
		...
		if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
			...
			if j == 1 then
				...
				if mod((j - 1), col) == 0 then
					...
					local anchorframe = ArkInventory.ContainerItemNameGet(loc_id, bar.item[j - col].bag,
						bar.item[j - col].slot)
					...
					local anchorframe = ArkInventory.ContainerItemNameGet(loc_id, bar.item[j - 1].bag, bar.item[j - 1]
					.slot)
...
function ArkInventory.Frame_Item_Update_Border(frame)
	...
	if obj then
		if ArkInventory.LocationOptionGet(loc_id, "slot", "border", "style") ~= ArkInventory.Const.Texture.BorderNone then
			local style = ArkInventory.LocationOptionGet(loc_id, "slot", "border", "style") or
			ArkInventory.Const.Texture.BorderDefault
			local file = ArkInventory.Lib.SharedMedia:Fetch(ArkInventory.Lib.SharedMedia.MediaType.BORDER, style)
			local size = ArkInventory.LocationOptionGet(loc_id, "slot", "border", "size") or
			ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].size
			local offset = ArkInventory.LocationOptionGet(loc_id, "slot", "border", "offset") or
			ArkInventory.Const.Texture.Border[ArkInventory.Const.Texture.BorderDefault].offset
...
function ArkInventory.Frame_Status_Update_Empty(loc_id, cp, ldb)
	...
	if ts == 0 then
		table.insert(y,
			string.format("%s%s%s", RED_FONT_COLOR_CODE, ArkInventory.Localise["STATUS_NO_DATA"], FONT_COLOR_CODE_CLOSE))
	else
		for t, e in ArkInventory.spairs(empty, function(a, b) return empty[a].type < empty[b].type end) do
			...
			if ldb then
				...
				if ArkInventory.db.char.option.ldb.bags.full then
					table.insert(y,
						string.format("%s%i/%i%s%s", c, e.count - e.empty, e.count, n, FONT_COLOR_CODE_CLOSE))
				else
...
function ArkInventory.Frame_Changer_Update(loc_id)
	if loc_id == ArkInventory.Const.Location.Bag then
		for bag_id in ipairs(ArkInventory.Global.Location[loc_id].Bags) do
			local frame = _G
			[ArkInventory.Const.Frame.Main.Name .. loc_id .. ArkInventory.Const.Frame.Changer.Name .. "WindowBag" .. bag_id]
...
function ArkInventory.Frame_Changer_Update_Bank()
	...
	for x = 1, ArkInventory.Global.Location[loc_id].bagCount do
		local frame = _G
		[ArkInventory.Const.Frame.Main.Name .. loc_id .. ArkInventory.Const.Frame.Changer.Name .. "WindowBag" .. x]
...
function ArkInventory.BagHighlight(frame, show)
	...
	if loc_id ~= nil and bag_id ~= nil then
		...
		local name = ArkInventory.Const.Frame.Main.Name ..
		loc_id .. ArkInventory.Const.Frame.Container.Name .. "Bag" .. bag_id
		local frame = _G[name]
		if not frame then
			return
		end
...
function ArkInventory.BlizzardAPIHooks(disable)
	...
	local tooltip_frames = { GameTooltip, ShoppingTooltip1, ShoppingTooltip2, ShoppingTooltip3, ItemRefTooltip,
		ItemRefShoppingTooltip1, ItemRefShoppingTooltip2, ItemRefShoppingTooltip3 }

	local tooltip_functions = {
		"SetAuctionItem", "SetAuctionSellItem", "SetAuctionCompareItem", "SetBagItem", "SetBuybackItem", "SetCraftItem",
		"SetCraftSpell", "SetGuildBankItem", "SetHyperlink",
		"SetHyperlinkCompareItem", "SetInboxItem", "SetInventoryItem", "SetLootItem", "SetLootRollItem",
		"SetMerchantCompareItem", "SetMerchantItem", "SetQuestItem",
		"SetQuestLogItem", "SetSendMailItem", "SetTradePlayerItem", "SetTradeSkillItem", "SetTradeTargetItem",
		"SetCurrencyToken", "SetBackpackToken", "SetMerchantCostItem",
		"SetText"
	}
...
function ArkInventory.ContainerNameGet(loc_id, bag_id)
	if loc_id ~= nil and bag_id ~= nil then
		local x = ArkInventory.Const.Frame.Main.Name ..
		loc_id .. ArkInventory.Const.Frame.Container.Name .. "Bag" .. bag_id
		return x
	end
end
...
function ArkInventory.LocationOptionGetReal_old2(loc_id, opt)
	...
	for i = 1, #opt do
		assert(p ~= nil,
			string.format(
			"code error: encountered nil at %s - please reload ui to reset database or report to author if it continues",
				s))
...
function ArkInventory.LocationOptionGetReal(loc_id, ...)
	...
	for i = 1, n do
		assert(p ~= nil,
			string.format(
			"bad author: %s is nil - please reload ui to reset the database or report to author if it continues", s))
...
function ArkInventory.LocationOptionSetReal(loc_id, ...)
	...
	for i = 1, n - 2 do
		assert(p ~= nil,
			string.format(
			"bad author: %s is nil - please reload ui to reset database or report to author if it continues", s))
...
function ArkInventory.ToggleChanger(loc_id)
	ArkInventory.LocationOptionSet(loc_id, "changer", "hide",
		not ArkInventory.LocationOptionGet(loc_id, "changer", "hide"))
	ArkInventory.Frame_Main_Generate(nil, ArkInventory.Const.Window.Draw.Refresh)
end
...
function ArkInventory.Frame_Vault_Log_Update()
	...
	local obj = _G
	[ArkInventory.Const.Frame.Main.Name .. loc_id .. ArkInventory.Const.Frame.Log.Name .. ArkInventory.Const.Frame.Scrolling.List]
	...
	for i = 1, numTransactions do
		...
		if GuildBankFrame.mode == "log" then
			...
			elseif type == "move" then
				msg = format(GUILDBANK_MOVE_FORMAT, name, itemLink, count, GetGuildBankTabInfo(tab1),
					GetGuildBankTabInfo(tab2))
			end
		elseif GuildBankFrame.mode == "moneylog" then
		...
		if msg then
			obj:AddMessage(msg ..
			GUILD_BANK_LOG_TIME_PREPEND .. format(GUILD_BANK_LOG_TIME, RecentTimeDate(year, month, day, hour)))
		end
	end
...
function ArkInventory.DisplayName1(p)
	...
	return string.format("%s\n%s > %s", p.name or ArkInventory.Localise["UNKNOWN"],
		p.faction_local or ArkInventory.Localise["UNKNOWN"], p.realm or ArkInventory.Localise["UNKNOWN"])
end

function ArkInventory.DisplayName2(p)
	-- switch menu
	return string.format("%s > %s > %s", p.realm or ArkInventory.Localise["UNKNOWN"],
		p.faction_local or ArkInventory.Localise["UNKNOWN"], p.name or ArkInventory.Localise["UNKNOWN"])
end
...
function ArkInventory.DisplayName4(p)
	-- switch character
	return string.format("%s%s (%s:%s) |cff7f7f7f[%s]|r", ArkInventory.ClassColourCode(p.class),
		p.name or ArkInventory.Localise["UNKNOWN"], p.class_local or ArkInventory.Localise["UNKNOWN"],
		p.level or ArkInventory.Localise["UNKNOWN"], p.faction_local or ArkInventory.Localise["UNKNOWN"])
end
