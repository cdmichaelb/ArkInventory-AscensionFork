							...
							for _, cat in ArkInventory.spairs( ArkInventory.Global.Category, function(a,b) return ArkInventory.Global.Category[a].fullname < ArkInventory.Global.Category[b].fullname end ) do
								...
								if cat_type == t then
									...
									if not def_bar then
										...
										n = string.format( "%s%s%s [%s]%s", LIGHTYELLOW_FONT_COLOR_CODE, cat.name, GREEN_FONT_COLOR_CODE, ArkInventory.CategoryLocationGet( loc_id, cat.id ), FONT_COLOR_CODE_CLOSE )
									end
					...
					if value == "DEBUG_INFO" then
						...
						if i.h then
							...
							if class == "item" then
								...
								ArkInventory.Lib.Dewdrop:AddLine( "text", string.format( "%s: %s%s (%s)", QUALITY, LIGHTYELLOW_FONT_COLOR_CODE, irar, _G["ITEM_QUALITY" .. irar .. "_DESC"] ) )
		...
		ArkInventory.Lib.Dewdrop:Open( frame,
			...
			"children", function( level, value )
				...
				if level == 2 and value and value > 0 then
					...
					ArkInventory.Lib.Dewdrop:AddLine(
						"text", "|cffff0000" .. ArkInventory.Localise["REMOVE"],
						"tooltipTitle", ArkInventory.Localise["REMOVE"],
						--"tooltipText", "",
						"func", function( )
				...
				if level == 1 then
					...
					for companionIndex = 1, n do
						...
						if ok then
							...
							if selected[companionSpellID] then
								companionName = string.format( "|cff00ff00%s|r", companionName )
								tooltipText = ArkInventory.Localise["CLICK_TO_DESELECT"]
							end
							
							if ( companionData and companionData.r ) then
								--companionName = string.format( "|cffff0000%s|r", companionName )
								tooltipText = string.format( "|cffff0000%s|r\n\n%s", ArkInventory.Localise["LDB_COMPANION_RESTRICTED"], tooltipText )
							end
							...
							ArkInventory.Lib.Dewdrop:AddLine(
								...
								"tooltipText", tooltipText,
								"checked", selected[companionSpellID],
								"func", function( )
									ArkInventory.db.char.option.ldb.pets.selected[companionSpellID] = not ArkInventory.db.char.option.ldb.pets.selected[companionSpellID]
					...
					if companionCount > 0 then
						...
						if checked then
							companionName = string.format( "|cff00ff00%s", companionName )
							tooltipText = nil
						end
			...
			"children", function( level, value )
				...
				if level == 2 and value then
					...
					for companionIndex = 1, n do
						...
						if ( not ok ) and ( companionData.usable[mountType] ) then
							ok = true
							speed = companionData.speed[mountType] or 0
						end
						...
						if ok then
							...
							if not ArkInventory.IsCataclysm then
								companionName = string.format( "%s [%s%%] ", companionName, speed )
							end
							...
							if ( companionData and companionData.r ) then
								--companionName = string.format( "|cffff0000%s|r", companionName )
								tooltipText = string.format( "%s%s%s\n\n%s", RED_FONT_COLOR_CODE, ArkInventory.Localise["LDB_COMPANION_RESTRICTED"], FONT_COLOR_CODE_CLOSE, tooltipText )
							end
							...
							ArkInventory.Lib.Dewdrop:AddLine(
								...
								"func", function( )
									local selected = ArkInventory.db.char.option.ldb.mounts[mountType].selected
									selected[companionSpellID] = not selected[companionSpellID]
					...
					if companionCount > 0 then
						...
						if allrandom and not ArkInventory.IsCataclysm then
							
							ArkInventory.Lib.Dewdrop:AddLine( )
							
							local slider_min = 60
							local slider_max = 100
							local slider_value = ArkInventory.db.char.option.ldb.mounts[mountType].min
							...
							ArkInventory.Lib.Dewdrop:AddLine(
								...
								"sliderFunc", function( v )
									ArkInventory.db.char.option.ldb.mounts[mountType].min = v
								end	
							)
